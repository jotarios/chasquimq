...
- **Phase 3 (first slice shipped):** Node.js bindings via `napi-rs`. `chasquimq-node` crate scaffolded; native `Producer` / `Consumer` / `Promoter` exposed via TSFN dispatch; high-level shim shipped (`Queue` / `Job` / `Worker` / `QueueEvents`); engine slices 8 (per-job retry overrides), 9 (events stream), 10 (repeatable jobs scheduler) wired. v1 follow-ups: per-job retry native exposure, IANA tz handling for repeatable jobs, catch-up policy for missed cron windows, parent/child flows.
