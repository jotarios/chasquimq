...
- **Phase 3 (first slice shipped):** Node.js bindings via `napi-rs`. `chasquimq-node` crate scaffolded; native `Producer` / `Consumer` / `Promoter` / `Scheduler` exposed via TSFN dispatch; high-level shim shipped (`Queue` / `Job` / `Worker` / `QueueEvents`); engine slices 8 (per-job retry overrides), 9 (events stream), 10 (repeatable jobs scheduler) wired; IANA timezone support landed (DST-aware via `chrono-tz`); catch-up policy for missed cron windows landed (`MissedFiresPolicy::Skip` (default) / `FireOnce` / `FireAll { max_catchup }` on `RepeatableSpec`); repeatable / cron native exposure shipped via PR #33 (`Queue.add({repeat})` upserts via `NativeProducer.upsertRepeatable`; `Worker` auto-spawns `NativeScheduler`, opt-out via `runScheduler: false`); per-job retry native exposure landed (`Queue.add({attempts, backoff})` honored end-to-end + `UnrecoverableError → HandlerError::unrecoverable` mapping). v1 follow-ups: parent/child flows.
