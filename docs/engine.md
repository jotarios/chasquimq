...
- **TLS via `rediss://`.** Pass `rediss://host:port` and the engine negotiates TLS via fred's `enable-native-tls` feature; trust roots come from the system TLS stack (Secure Transport on macOS, OpenSSL on Linux, Schannel on Windows). For private CAs, install them in the system store. Plain `redis://` is unaffected — the TLS connector is only constructed when the URL scheme demands it.
