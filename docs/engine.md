...
**Per-job and per-fire overrides.** A `JobRetryOverride` carried in the job's encoded payload (`max_attempts` and/or `backoff`) wins over the queue-wide config for that job. The one-off producer surfaces it via `AddOptions::with_retry` (`add(name, data, { attempts, backoff })` on the shims). Repeatable specs surface the same override via `RepeatableSpec::with_retry` (`attempts` / `backoff` on `upsert_repeatable`): the scheduler threads it onto every job it fires from the spec, so a per-spec `max_attempts` / `backoff` applies to all fired jobs exactly as a per-job override would. Leave it unset to inherit the queue-wide policy.
...
Optional per-queue **global** rate limit: at most `max` **jobs** admitted per `duration_ms` window, shared across **all** workers on the queue. Configure via `ConsumerConfig::rate_limit: Option<RateLimit>` where `RateLimit { max: u32, duration_ms: u64, group_key: Option<String> }`. `None` (the default) is zero-overhead — the gate is a single skipped `if let Some` branch per batch, no EVALSHA, no allocation.

**One global bucket, not one per worker.** A single token bucket lives in Redis under `{chasqui:<queue>}:limiter` (a hash carrying the token count `t` and last-refill ms `ts`). Because the key carries the same `{chasqui:<queue>}` hash tag as the stream, it co-locates on one Cluster slot (`ClusterHash::FirstKey` is correct). N workers on the queue draw from that one budget — `max` is a queue-wide ceiling, not a per-worker one.

**Token-bucket algorithm, evaluated once per read attempt.** Before each `XREADGROUP`, the reader runs one `EVALSHA` against the bucket script — **one round trip per read attempt, never per job**. The script uses `redis.call('TIME')` as the shared clock (the same pattern as `PROMOTE_SCRIPT`), so there is no per-worker clock skew: every worker refills and spends against the same server-side timestamp. If a token is available the script decrements and returns `0` (granted); otherwise it returns a positive `wait_ms`. The reply is parsed **fail-closed** — any shape the Rust side cannot read as a non-negative integer is treated as "throttle" (a gate that failed open would silently over-admit).

**Delay the whole read; FIFO is preserved.** When throttled, the reader sleeps `wait_ms` at the **batch boundary** and re-checks — no job is reordered, held, or requeued. The gate sits before `XREADGROUP` (after the pause gate — a paused queue must not spend a token), so once a token is granted the batch reads and dispatches in stream order exactly as an un-limited reader would. The re-check never assumes a token is ready after sleeping (concurrent workers may have taken it); it re-runs the script.

**Cold-start burst allowance.** A fresh or idle bucket starts full, so the first window admits up to `max` jobs immediately before settling to the `max`/`duration` steady state. This is standard token-bucket behavior — the first second after a queue goes live (or wakes from idle) can pass up to `max` jobs, not `max`-per-`duration` averaged. State it in monitoring so a first-window burst isn't mistaken for a leak. (The bucket key's `PEXPIRE` is `duration_ms * 2` and is refreshed on every acquire, so an idle bucket that expires is indistinguishable from a lazily-refilled full one — no lost-state burst.)

**CPU cost, honestly.** At coarse rates (e.g. hundreds/sec) a throttled reader parks in a `wait_ms`-long sleep and wakes only when a token is due — near-zero CPU while throttled. At **very high** `max`/second the deficit-of-one wait clamps to ~1 ms, so the reader re-checks about every millisecond, firing one cheap `EVALSHA` per re-check. That is real (small) Redis load, not "zero CPU" — and N throttled workers on the same `wait_ms` wake together. A `min_poll_ms` sleep floor is a tracked follow-up if very-high-rate limiters ever need it.

**Groups are reserved, not shipped.** `group_key` (`RateLimit::group_key`, Node `limiter.groupKey`, Python `rate_limit_group_key`) is parsed but **rejected** by `ConsumerConfig::validate()` with "not supported in this version (global per-queue limiter only)". Per-key (name-scoped) sub-buckets are a documented future follow-up; reserving the field now keeps that addition non-breaking.

**Observability.** Each throttle sleep emits `MetricsSink::rate_limited_tick(RateLimitedTick { wait_ms })`; the `chasquimq-metrics` adapters expose `chasquimq_rate_limited_total` (counter) and `chasquimq_rate_limit_wait_seconds` (histogram). On the events stream, `e=rate-limited` (field `wait_ms`) fires once per throttle *entry* per reader — with M workers on one queue, expect up to M events per throttle episode, and a bucket flapping around `tokens ≈ 1` may re-emit; aggregate rather than assume exactly-one.
...
- **A repeatable spec with a per-fire retry override needs a scheduler-first deploy.** The stored spec (`StoredSpec`, the msgpack positional array in `{chasqui:<queue>}:repeat:spec:<key>`) gained a trailing-optional `retry` field for the per-fire override. To keep that the sole trailing-optional slot, `missed_fires` became always-serialized — so a default-policy spec now encodes as a 9-field array (one field wider than the historical 8-field legacy shape; old readers tolerate the extra trailing field, and a missing one defaults to `Skip`). A spec carrying `retry = Some(...)` is a 10-element array a pre-this-change scheduler cannot decode. Roll out new schedulers *before* writing retry-bearing specs. A spec is re-encoded on every `upsert_repeatable`, so a stale scheduler only breaks if a *new* retry-bearing spec is written while it is still running — the same deploy-order contract as `Job::retry`. Retry-less, default-policy specs are back-compatible in both directions.
...
- **The rate limiter admits a first-window burst.** A fresh/idle limiter bucket starts full, so the first `duration_ms` window after a queue goes live (or wakes from idle) can admit up to `max` jobs before the `max`/`duration` steady state kicks in — standard token-bucket behavior, not a bug. Near-zero CPU while throttled at coarse rates; at very high `max`/second the reader re-checks roughly every ~1 ms (one cheap `EVALSHA` per re-check). The limiter is a *global per-queue* budget shared across all workers; `group_key` is reserved and rejected. The CLI does **not** surface the limiter — `chasqui inspect` / `watch` read Redis keys with no worker config, and the raw bucket hash (`t`/`ts`) is meaningless without the `max`/`duration` that live in worker config, so a "limiter depth" line would be misleading (present-but-unset would look identical to unlimited).
...
- **Rate limiting delays the whole read, not each job.** A per-queue limiter that ran a check per job would add exactly the per-job round trip the engine exists to avoid. Instead the token bucket is evaluated once per read attempt (one `EVALSHA` before `XREADGROUP`), the throttle sleeps the whole batch at the boundary, and FIFO is preserved because no job is reordered or requeued. `redis.call('TIME')` supplies a shared clock so N workers spend against one global bucket with no per-worker skew. When the limiter is unset the path is one skipped branch.
