...
- **TLS via `rediss://`.** The engine ships with fred's `enable-rustls-ring` feature on, so any `rediss://host:port` URL passed to `Producer::connect` / `Consumer::connect` negotiates TLS end-to-end against the system trust store (rustls-native-certs reads keychain on macOS, the system CA bundle on Linux, and schannel on Windows; ElastiCache and MemoryDB use AWS Trust CA, which is already trusted on every supported platform). No extra config knobs — pass the URL and you're done. Plain `redis://` is unaffected: the TLS connector is only constructed when the URL scheme demands it. For private CAs, point `SSL_CERT_FILE` at a PEM bundle; rustls-native-certs picks it up first on every platform.
...
- **`IDMP` idempotent `XADD` (Redis 8.6).** DLQ relocation is retry-safe at the Redis layer; producer retries after network blips don't double-publish. Stable `jobId` on the immediate path scopes dedup to the producer instance via Redis `IDMP <producer_id> <job_id>`.
