...
- **Node `NativeJob` dropped from top-level export (2026-05-08, breaking).** Following the Python lead, the native binding's `Job` value-type is no longer re-exported from the `chasquimq` package root. There is one user-facing `Job` — the high-level class returned by `Queue.add` and passed to your `Worker` processor. Power users who need the native value-type can still import it from `chasquimq/index.js` (the underlying napi binding). README and `__test__/native.test.ts` updated to reach into the napi binding directly with a leading-underscore alias.
