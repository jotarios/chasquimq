...
## Per-fire retry overrides on repeatable jobs (cross-FFI slice)

**Date:** 2026-06-25. **Branch:** `feat/repeatable-retry-overrides`.

Closes a produce-side asymmetry: a normal `add(name, data, { attempts, backoff })` carries a per-job [`JobRetryOverride`](https://github.com/jotarios/chasquimq/blob/main/chasquimq/src/job.rs) that wins over the worker's queue-wide retry config, but a *repeatable* upsert silently dropped the same `attempts` / `backoff` at the wire layer — every fired job inherited the queue-wide budget no matter what the spec asked for. Now `attempts` / `backoff` on `Queue.upsertRepeatableJob` / `upsert_repeatable_job` (and on `add(name, data, { repeat, attempts, backoff })`) set a **per-fire retry override**: every job the scheduler mints from that spec carries the override in its `Job<T>` payload, so its per-fire `max_attempts` / `backoff` win over the queue-wide config exactly like a one-off `add`.

**The consumer was already correct.** `consumer/worker.rs` already honored a job's in-payload `retry` (per-job `max_attempts` / `backoff` win over queue-wide config) for one-off adds. This slice only had to *thread an existing override onto the fired job* — no consumer change, and `SCHEDULE_REPEATABLE_SCRIPT` (which routes pre-encoded `Job<T>` bytes) is untouched because the override lives inside the msgpack body. The scheduler's `encode_fire` gained a `retry: Option<JobRetryOverride>` parameter and sets `job.retry` before encoding; `None` keeps the legacy 4-field `Job<T>` wire shape byte-identically.

**StoredSpec wire-format change (Option A).** `RepeatableSpec<T>` gained a trailing `retry: Option<JobRetryOverride>` field (`with_retry()` builder for parity with `AddOptions::with_retry`), and the on-wire `StoredSpec` (the msgpack positional array persisted in `{chasqui:<queue>}:repeat:spec:<key>`) changed in two coupled ways:

- `missed_fires` is **now always serialized** (`#[serde(default)]` only — its `skip_serializing_if` was dropped). It used to be the lone trailing-optional slot; a default-policy (`Skip`) spec therefore now encodes as a 9-field array, one field wider than the historical 8-field legacy shape.
- `retry` is **appended last** as the new trailing-optional (`skip_serializing_if = Option::is_none`), so a retry-less spec keeps the 9-field shape.

Why drop the skip on `missed_fires`? A positional array can carry **at most one** trailing conditionally-omitted field. Two independent `skip_serializing_if` fields at the tail create a *hole*: when `missed_fires` is skipped (`Skip`) but `retry` is `Some`, the retry value lands in the `missed_fires` slot at decode time and corrupts (or fails) the decode. Making `missed_fires` unconditional pins its position so `retry` can own the trailing-optional slot cleanly. Old specs already in Redis still decode (the 8-field legacy shape → `missed_fires = Skip`, `retry = None`; a 9-field non-default-policy spec keeps its policy intact) — verified by an exhaustive back-compat test set in `chasquimq/src/repeat.rs` covering: legacy-default decode, legacy-non-default-policy decode (the anti-corruption guard — proves `FireAll` survives the layout change), `missed_fires=Skip + retry=Some` round-trip (the anti-hole guard), `retry=None` omits the trailing field, and a downgrade-read (an old-shape reader decoding a new default spec). The pre-existing `omits_field_on_wire` test was updated, not deleted — its byte-identity assertion was intentionally superseded by Option A.

**Deploy-order requirement** (mirrors the `Job::retry` rule in `job.rs`). A `StoredSpec` with `retry = Some(...)` is a 10-element array a pre-this-slice scheduler cannot decode (it rejects the array length). Roll out new schedulers *before* writing retry-bearing specs. A spec is re-encoded on every `upsert_repeatable`, so a stale scheduler only breaks if a *new* (retry-bearing) spec is written while it is still running — same contract as `Job::retry`. Default-policy retry-less specs are back-compatible in both directions.

**`job_id` on a repeatable upsert is rejected loudly.** Passing `jobId` (Node) / `job_id` (Python) — including the nested `RepeatOptions.jobId` on the Node `add` path — raises `NotSupportedError` rather than being silently dropped: the scheduler mints a fresh ULID per fire, so a "stable" id the caller might rely on for dedup would be a correctness foot-gun. Stable-id-per-fire is a deferred follow-up (see below); rejecting now is reversible when that slice lands. The auto-derived `repeatJobKey` (the spec key, distinct from a per-fire job id) is the supported handle and is unaffected.

Both shims reuse the **exact** `attempts` / `backoff` → native-retry translation the normal `add` path uses (Node `buildRetryOverride` → `to_engine_retry_override`; Python `_build_retry_override` → `dict_to_retry_override`), so the two surfaces can't drift — `to_engine_retry_override` was extracted on the Node side as the single source of truth for the FFI retry shape.

Tests: engine `chasquimq/tests/repeatable.rs` (upsert with `attempts: 1` + always-failing handler → exactly one attempt → DLQ-as-`retries_exhausted`, proving the override beats a generous queue-wide budget; plus a custom-backoff variant) and the wire-format back-compat set above; Node `__test__/repeatable.test.ts` and Python `tests/test_high_level.py` mirror the override-honored + `jobId`/`job_id`-rejected cases symmetrically.

Doc surfaces synced: this slice, `docs/engine.md` (per-fire override note in Retry semantics + the StoredSpec layout / deploy-order operational-notes bullet), both shim READMEs (symmetric "Repeatable jobs" sections), `site/src/content/docs/reference/{node-api,python-api}.md` (`attempts` / `backoff` per-fire on repeatable + the `jobId` / `job_id` `NotSupportedError` behavior), `reference/options.md` (per-fire retry rows under repeatable), `reference/wire-format.md` (the StoredSpec layout + deploy-order rule), and `guides/schedule-repeatable-jobs.mdx` (a per-fire-retry subsection replacing the now-false "not threaded yet" gotcha).

## Per-queue rate limiting (cross-FFI slice)

**Date:** 2026-07-07. **Branch:** `feat/rate-limiting`.

Ships an optional **global per-queue** rate limiter — the "throttle this queue to N/sec regardless of how many workers" capability. `max` is a ceiling on **jobs** admitted per `duration` window, shared across *all* workers on the queue: a single token bucket lives in Redis at `{chasqui:<queue>}:limiter` (a hash of token count `t` + last-refill ms `ts`), and N workers draw from that one budget, not N. The key carries the queue's `{chasqui:<queue>}` hash tag, so it co-locates on one Cluster slot (`ClusterHash::FirstKey` correct, no new cluster machinery).

**Job-metering semantic (stated precisely because it's the thing people get wrong).** The bucket meters *jobs*, and it is evaluated **once per reader read attempt** — one `EVALSHA` before each `XREADGROUP`, **never per job**. A granted read dispatches a whole batch; a throttled read sleeps `wait_ms` at the batch boundary and re-checks. No job is reordered, held, or requeued, so **FIFO is preserved**. The gate sits after the pause gate (a paused queue must not spend a token) and before the read. The script uses `redis.call('TIME')` as a shared clock (same pattern as `PROMOTE_SCRIPT`), eliminating per-worker clock skew.

**Fail-closed reply parsing (correctness BLOCKER caught in review).** A dedicated `parse_rate_limit_reply` maps `Integer(n>=0)` / u64-parseable `String`/`Bytes` to `n`, and **any other shape to a full-window throttle** — it does not reuse `value_as_u64` (which maps every non-`Integer` shape to `0` = "granted", silently disabling the limiter under RESP3 float/string framing). The Lua returns `math.floor` integer framing to keep the reply an `:int`. A gate that fails *open* silently over-admits, which is worse than no limiter.

**Cold-start burst allowance (documented on every surface).** A fresh or idle bucket starts full, so the first window admits up to `max` jobs before settling to the `max`/`duration` steady state — standard token-bucket behavior. The bucket key's `PEXPIRE` is `duration_ms * 2`, refreshed on every acquire, so an idle bucket that expires is indistinguishable from a lazily-refilled full one (no lost-state burst).

**CPU honesty.** Near-zero while throttled at coarse rates (the reader parks in a `wait_ms` sleep). At very high `max`/second the deficit-of-one wait clamps to ~1 ms, so the reader re-checks about every millisecond — one cheap `EVALSHA` per re-check (real but small Redis load, and N throttled workers on the same `wait_ms` wake together). A `min_poll_ms` sleep floor is a tracked follow-up.

**Groups reserved, not shipped.** `group_key` (Rust) / `limiter.groupKey` (Node) / `rate_limit_group_key` (Python) is parsed but **rejected** with "not supported in this version (global per-queue limiter only)". Name-scoped sub-buckets are a documented additive follow-up; reserving the field keeps that non-breaking.

Engine (`chasquimq`):

- `config.rs`: new `RateLimit { max: u32, duration_ms: u64, group_key: Option<String> }` (no `Default`); `ConsumerConfig::rate_limit: Option<RateLimit>` (default `None`, hand-written `Debug` field added, `validate()` rejects `max == 0` / `duration_ms == 0` / `group_key = Some`).
- `redis/keys.rs`: `limiter_key(queue)` → `{chasqui:<queue>}:limiter` (hash-tag co-location test).
- `redis/commands.rs`: `RATE_LIMIT_ACQUIRE_SCRIPT` (`#!lua flags=allow-oom`, token-bucket refill, integer-framed return), `evalsha_/eval_rate_limit_args` builders, and the fail-closed `parse_rate_limit_reply`.
- `consumer/reader.rs`: `RateLimitGate` (loop-local SHA cache + NOSCRIPT self-heal modeled on `dlq.rs`, `sleep_or_shutdown` for prompt shutdown-while-throttled, a `throttled` latch so events fire once per throttle entry) threaded into `reader_loop` between the pause gate and `XREADGROUP`. Unset = one skipped `if let Some` branch, zero overhead.
- `metrics/mod.rs`: `MetricsSink::rate_limited_tick(RateLimitedTick { wait_ms })` (default no-op) + `RateLimitedTick` re-exported from the crate root; `metrics/testing.rs` `InMemorySink` records ticks (`rate_limited_ticks()` accessor).
- `events.rs`: `emit_rate_limited(wait_ms)` → `e=rate-limited` (field `wait_ms`), queue-scoped, once per throttle entry per reader; schema-table row added.

Metrics adapter (`chasquimq-metrics`): `rate_limited_tick` wired in `facade.rs` (`chasquimq_rate_limited_total` counter + `chasquimq_rate_limit_wait_seconds` histogram) and `labeled.rs` (same, plus `queue` label + delegate to inner), and two doc rows in `lib.rs`. **Sibling fix in the same slice:** the pre-existing-but-dropped `stalled_tick` (the engine fires it but both Prometheus adapters silently swallowed it — a confirmed gap since the stalled-detector slice) was wired into `facade.rs` + `labeled.rs` + the `lib.rs` doc table while these files were open.

Node shim (`chasquimq-node`): `WorkerOptions.limiter?: { max, duration, groupKey? }`, mapped to the nested NAPI `LimiterOpts`; constructor validates both fields present + positive, and rejects `groupKey` in JS (mirroring the engine message) rather than as an opaque native error. The `rateLimit()` stub (BullMQ's *manual per-invocation* throttle-until call) is left throwing `NotSupportedError` — semantically distinct from the constructor `limiter`.

Python shim (`chasquimq-py`): flat `Worker(rate_limit_max=, rate_limit_duration_ms=, rate_limit_group_key=)` kwargs riding `consumer_kwargs`; PyO3 lands them on `cfg.rate_limit`, rejecting `rate_limit_group_key` and `rate_limit_duration_ms`-without-`rate_limit_max`. Naming asymmetry is deliberate: Node nests like `RetryOpts`, Python is flat snake_case like `max_stalled_attempts`; both land on the same engine `ConsumerConfig.rate_limit`.

**CLI is out of scope (explicit non-goal).** `chasqui inspect` / `watch` build their snapshot from `XLEN`/`XPENDING`/`ZCARD` with **no worker config**; the limiter's `max`/`duration` live in worker config, not Redis, and the raw bucket hash (`t`/`ts`) is meaningless without them (present-but-unset would look identical to unlimited). No limiter line is added to inspect/watch.

Tests: engine `chasquimq/tests/rate_limit.rs` (live Redis) — single worker respects the rate, two workers share ONE global bucket (the definitive not-per-process check), bounded tick count while throttled (~0-CPU proof), prompt shutdown-while-throttled, metrics + `e=rate-limited` on throttle, and the fail-closed `parse_rate_limit_reply` unit test (every non-integer shape → throttle, the regression test for the review BLOCKER); `config.rs` unit tests reject `max=0` / `duration=0` / `group_key` and accept a valid global limiter + `None`. Cross-shim: a Node worker + a Python worker on the SAME queue share one bucket. Bench guard: a limiter-configured-but-unthrottled `worker-concurrent` stays within a few % of the no-limiter baseline (rebuts "adds per-job round trips").

Doc surfaces synced: this slice, `README.md` (feature table `Rate limiter | ✓ (global per-queue)` + dropped from the Future list), `docs/engine.md` (new "Rate limiting" section + operational-notes bullet + "Why the design choices" bullet), both shim READMEs (symmetric "Rate limiting" sections + Worker box-table rows), `site/src/content/docs/concepts/{performance-trade-offs,architecture-decisions}.md` (rewrote the arguments-against-a-limiter into the shipped design), new `concepts/rate-limiting.md` (registered in `astro.config.mjs`), `reference/options.md` (new "Rate limiting" cross-language table), `reference/{node-api,python-api,rust-api}.md` (`limiter` / `rate_limit_*` / `RateLimit` + `RateLimitedTick`), `reference/error-codes.md` (CMQ-101 reworded — reserved for the manual `rateLimit()` call, not the shipped constructor `limiter`), and `guides/migrate-from-sidekiq-celery.mdx` (Sidekiq/Celery rate-limit rows now map to the shipped global limiter). `guides/migrate-from-bullmq.mdx` intentionally left untouched — it had no rate-limiter claim to correct, and adding a BullMQ-comparison row would introduce new positioning prose (out of scope by the no-BullMQ-marketing rule).
...
- **Stable `job_id` per repeatable fire.** The scheduler mints a fresh ULID for each fired job, so a caller-supplied stable id has no place to stick yet; passing `jobId` / `job_id` to a repeatable upsert currently raises `NotSupportedError` (see the [Per-fire retry overrides](#per-fire-retry-overrides-on-repeatable-jobs-cross-ffi-slice) slice above). A future slice would need an iteration-suffix scheme that interacts with catch-up fires and the idempotency markers; the loud rejection is reversible when that lands.
- **Name-scoped rate-limit groups.** The [rate-limiting slice](#per-queue-rate-limiting-cross-ffi-slice) shipped a *global* per-queue token bucket; the `group_key` / `limiter.groupKey` / `rate_limit_group_key` field is reserved and rejected today. A follow-up would bucket by `entry.name` at the reader gate (one extra Lua invocation guarded by `group_key.is_some()`), giving a separate budget per job kind. Additive — the reserved-field rejection flips to a wire-through, no new field.
- **`min_poll_ms` sleep floor for very-high-rate limiters.** At very high `max`/second the rate limiter's deficit-of-one `wait_ms` clamps to ~1 ms, so N throttled workers re-check about every millisecond (one cheap `EVALSHA` each). A configurable minimum poll interval would coarsen the re-check cadence at the cost of admission granularity; not built until a real high-rate workload needs it.
