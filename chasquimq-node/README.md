...
> **Status:** Phase 3 complete. The high-level shim ships `Queue` / `Worker` / `Job` / `QueueEvents` (with per-job retries, repeatable / cron jobs, DLQ peek/replay, delayed cancel, and `UnrecoverableError` short-circuit to DLQ); the unwrapped engine bindings (`Producer` / `Consumer` / `Promoter` / `Scheduler`) are re-exported from the same `chasquimq` namespace. Public API is pre-1.0 and may change.
...
## Two API layers, one import

```ts
// High-level Queue/Worker shim (TypeScript)
import { Queue, Worker } from "chasquimq";

// Direct NAPI bindings to the Rust engine — same package, no subpath
import { Producer, Consumer, Promoter, Scheduler } from "chasquimq";
```
