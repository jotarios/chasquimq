...
Prebuilt binaries ship for darwin / linux / win32 on arm64 + x64. There is no install-time compilation.
...
cargo add tokio-util bytes serde --features serde/derive
```
...
Save this as `hello.{js,py,rs}` and run it.
...
# processing welcome: {"to":"ada@example.com"}
```
...
async fn main() -> anyhow::Result<()> {
    ...
    consumer.run(|job| async move {
        println!("processing: {}", job.payload.to);
        Ok(bytes::Bytes::new())
    }, cancel).await?;
...
You should see one line of output. That is your first job round-tripping through Redis Streams.
