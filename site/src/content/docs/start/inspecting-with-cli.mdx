...
The CLI ships as a separate Rust crate so you don't drag operator tooling into your application bindings.

```bash
cargo install chasquimq-cli
```

That puts a `chasqui` binary on your `$PATH`. If you don't have Rust, prebuilt binaries land on the [GitHub Releases page](https://github.com/jotarios/chasquimq/releases) under `chasquimq-cli-v*`.
