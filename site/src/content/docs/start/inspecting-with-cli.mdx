...
**Prebuilt binary (recommended).** One-liner installer covers macOS (Apple Silicon + Intel), Linux (x86_64 + aarch64), and Windows:

```bash
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/jotarios/chasquimq/releases/latest/download/chasquimq-cli-installer.sh | sh
```

Or download a platform-specific tarball from the [Releases page](https://github.com/jotarios/chasquimq/releases) and extract `chasqui` somewhere on your `$PATH`.

**From source** (you have Rust 1.85+ already):
