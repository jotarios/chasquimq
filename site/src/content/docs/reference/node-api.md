...
- `maxStalledCount` — **v1.4 breaking change.** Now routes to engine `max_stalled_attempts` (the stalled-detector ceiling — stall cycles past `idle_threshold_ms` before DLQ-as-`stalled`). **Default `1`.** Pre-v1.4 this field was mis-routed to `max_attempts` with a `?? 3` fallback. Migration: use `maxAttempts` for the old "cap total attempts" semantic. A one-time `WARN [chasquimq]` log fires per process when `maxStalledCount` is set without `maxAttempts`.
