...
use crate::redis::commands::{
    REPLAY_DLQ_SCRIPT, eval_replay_args, evalsha_replay_args, script_load_args, xadd_args,
    xrange_args, zadd_delayed_args,
};
...
impl<T: Serialize> Producer<T> {
    ...
    pub async fn peek_dlq(&self, limit: usize) -> Result<Vec<DlqEntry>> {
        let entries = self.xrange_dlq(limit).await?;
        Ok(entries.into_iter().map(parse_dlq_entry).collect())
    }

    async fn xrange_dlq(&self, limit: usize) -> Result<Vec<XrangeEntry>> {
        let client = self.pool.next_connected();
        let cmd = CustomCommand::new_static("XRANGE", ClusterHash::FirstKey, false);
        let res: Value = client
            .custom(cmd, xrange_args(self.dlq_key.as_ref(), limit))
            .await
            .map_err(Error::Redis)?;
...
fn parse_dlq_entry(entry: XrangeEntry) -> DlqEntry {
