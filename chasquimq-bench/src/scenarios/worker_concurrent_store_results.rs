...
use super::worker_generic::drive_worker_scenario_with_handler;
use crate::sample::{Payload, generate_sample};
use chasquimq::config::ConsumerConfig;

const RESULT_BYTES: &[u8; 64] = b"chasquimq-bench-result-payload-64-bytes-fixed-size-XXXXXXXXXXXXX";
...
pub async fn run(redis_url: &str, queue: &str, scale: u32) -> ScenarioReport {
    ...
    drive_worker_scenario_with_handler(
        ...
        || chasquimq::Bytes::from_static(RESULT_BYTES),
    )
    .await
}
